...
import it.cnr.istc.pst.exploraa.server.LessonManager.LessonManagerListener;
...
public class LessonController {
    ...
    static void createLesson(final Context ctx) {
        ...
        lesson_manager.addLessonManagerListener(new LessonManagerListener() {

            @Override
            public void newToken(final Token token) {
                try {
                    MessagingService.getInstance().publish("user/" + teacher_id + "/input",
                            App.MAPPER.writeValueAsString(token), false);
            ...
            @Override
            public void stateChanged(LessonState state) {
                MessagingService.getInstance().publish("lesson/" + lesson_entity.getId() + "/state", state.name(),
                        true);
            }

            @Override
            public void timeChanged(long time) {
                MessagingService.getInstance().publish("lesson/" + lesson_entity.getId() + "/time", Long.toString(time),
                        true);
            }
        });
    ...
    static void followLesson(final Context ctx) {
        try {
            ...
            final Set<String> interests = App.MAPPER.readValue(ctx.formParam("lesson_id"),
