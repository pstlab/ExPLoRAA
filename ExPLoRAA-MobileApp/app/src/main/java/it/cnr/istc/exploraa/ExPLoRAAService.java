...
public class ExPLoRAAService extends Service implements LocationListener {
    ...
    private static final String CHANNEL_ID = "ExPLoRAA Channel";
    ...
    private final Collection<ExPLoRAAContext.StimuliListener> stimuli_listeners = new ArrayList<>();
    private final Collection<ExPLoRAAContext.FollowingLessonsListener> following_lessons_listeners = new ArrayList<>();
    private final Collection<ExPLoRAAContext.TeachersListener> teachers_listeners = new ArrayList<>();
    private final Collection<ExPLoRAAContext.TeachingLessonsListener> teaching_lessons_listeners = new ArrayList<>();
    private final Collection<ExPLoRAAContext.StudentsListener> students_listeners = new ArrayList<>();
    ...
    public void setUser(User user) {
        if (this.user != user) {
            if (this.user != null) {
                ...
                try {
                    ...
                    if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED)
                        ((LocationManager) getSystemService(Context.LOCATION_SERVICE)).removeUpdates(ExPLoRAAService.this);
                    par_types.clear();
                    id_par_types.clear();
                    stimuli.clear();
                    for (ExPLoRAAContext.StimuliListener l : stimuli_listeners) l.stimuliCleared();
                    ...
                    for (ExPLoRAAContext.FollowingLessonsListener l : following_lessons_listeners)
                    ...
                    for (ExPLoRAAContext.TeachersListener l : teachers_listeners)
                        l.teachersCleared();
                    ...
                    for (ExPLoRAAContext.TeachingLessonsListener l : teaching_lessons_listeners)
                    ...
                    for (ExPLoRAAContext.StudentsListener l : students_listeners)
                        l.studentsCleared();
            ...
            if (user != null) try {
                            ...
                            case Stimulus:
                                ...
                                switch (stimulus.stimulus_type) {
                                    case Text:
                                        ...
                                        content = ((Message.Stimulus.TextStimulus) stimulus).content;
                                        break;
                                    case Question:
                                        title = getString(R.string.question);
                                        content = ((Message.Stimulus.QuestionStimulus) stimulus).question;
                                        break;
                                    case URL:
                                        title = getString(R.string.url);
                                        content = ((Message.Stimulus.URLStimulus) stimulus).content;
                                        break;
                                }
                                NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(ExPLoRAAService.this, CHANNEL_ID)
                ...
                if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED)
                    ((LocationManager) getSystemService(Context.LOCATION_SERVICE)).requestLocationUpdates(LocationManager.GPS_PROVIDER, 0, 0, ExPLoRAAService.this);
    ...
    void addStimulus(@NonNull final Message.Stimulus stimulus) {
        ...
        for (ExPLoRAAContext.StimuliListener listener : stimuli_listeners)
    ...
    private void addFollowingLesson(@NonNull final FollowingLessonContext l_ctx) {
        ...
        FollowingLessonContext.FollowingLessonListener following_lesson_listener = new FollowingLessonContext.FollowingLessonListener() {
            ...
            public void timeChanged(long t) {
                for (ExPLoRAAContext.FollowingLessonsListener listener : following_lessons_listeners)
    ...
    private void addTeachingLesson(@NonNull final TeachingLessonContext l_ctx) {
        ...
        TeachingLessonContext.TeachingLessonListener l = new TeachingLessonContext.TeachingLessonListener() {
            @Override
            public void timeChanged(long t) {
                for (ExPLoRAAContext.TeachingLessonsListener listener : teaching_lessons_listeners)
    ...
    private void addTeacher(@NonNull final TeacherContext t_ctx) {
        ...
        TeacherContext.TeacherListener l = new TeacherContext.TeacherListener() {
            ...
            public void onlineChanged(boolean on_line) {
                for (ExPLoRAAContext.TeachersListener listener : teachers_listeners)
    ...
    void addStudent(@NonNull final StudentContext s_ctx) {
        ...
        StudentContext.StudentListener l = new StudentContext.StudentListener() {
            @Override
            public void onlineChanged(boolean on_line) {
                for (ExPLoRAAContext.StudentsListener listener : students_listeners)
            ...
            protected Void doInBackground(Object... objects) {
                try {
                    ...
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ExPLoRAAService.this, e.getLocalizedMessage(), Toast.LENGTH_LONG).show();
                        }
                    });
                }
                return null;
    ...
    @SuppressLint("StaticFieldLeak")
    public Collection<Lesson> getLessons() throws ExecutionException, InterruptedException {
    ...
    @SuppressLint("StaticFieldLeak")
    public boolean login(@NonNull final String email, @NonNull final String password) throws ExecutionException, InterruptedException {
    ...
    public void addStimuliListener(ExPLoRAAContext.StimuliListener l) {
        stimuli_listeners.add(l);
    }

    public void removeStimuliListener(ExPLoRAAContext.StimuliListener l) {
        stimuli_listeners.remove(l);
    }

    public void addFollowingLessonsListener(ExPLoRAAContext.FollowingLessonsListener l) {
        following_lessons_listeners.add(l);
    }

    public void removeFollowingLessonsListener(ExPLoRAAContext.FollowingLessonsListener l) {
        following_lessons_listeners.remove(l);
    }

    public void addTeachersListener(ExPLoRAAContext.TeachersListener l) {
        teachers_listeners.add(l);
    }

    public void removeTeachersListener(ExPLoRAAContext.TeachersListener l) {
        teachers_listeners.remove(l);
    }

    public void addTeachingLessonsListener(ExPLoRAAContext.TeachingLessonsListener l) {
        teaching_lessons_listeners.add(l);
    }

    public void removeTeachingLessonsListener(ExPLoRAAContext.TeachingLessonsListener l) {
        teaching_lessons_listeners.remove(l);
    }

    public void addStudentsListener(ExPLoRAAContext.StudentsListener l) {
        students_listeners.add(l);
    }

    public void removeStudentsListener(ExPLoRAAContext.StudentsListener l) {
        students_listeners.remove(l);
    }

    @SuppressLint("StaticFieldLeak")
    public boolean newUser(@NonNull final String email, @NonNull final String password, @NonNull final String first_name, @NonNull final String last_name) throws ExecutionException, InterruptedException {
        return new AsyncTask<String, Integer, Boolean>() {
            @Override
            protected Boolean doInBackground(String... strings) {
                try {
